<template>
  <ArtPermissionGuard permission="SmisToolPersonalRequisition:View">
    <div class="tool-requisition-page business-workspace-page art-full-height">
      ...
      <ArtTableQuery
        ...
        :table-props="{
          ...
          tableLayout: 'fixed',
          emptyText: '暂无个人领用明细',
          emptyDescription: '先维护工器具个人标准和领用计划，再生成到期领用单。'
        }"
        focusable
      />
...
<script setup lang="tsx">
  ...
  import {
    ...
    type SmisToolPersonalRequisitionItem,
    type SmisToolPersonalRequisitionOverview,
    type SmisToolPersonalRequisitionSearchParams,
    type SmisToolScopeOption
  } from '@smis/api'
  import IssuanceStatisticsDialog from '../tool-issuance-record/modules/issuance-statistics-dialog.vue'
  ...
  defineOptions({ name: 'SmisToolPersonalRequisition' })

  type TableParams = SmisToolPersonalRequisitionSearchParams & {
    current?: number
    size?: number
    page?: number
    pageSize?: number
  ...
  interface PushDialogExpose {
    handleOpen: (rows: SmisToolPersonalRequisitionItem[]) => Promise<void>
  }
  ...
  const searchQuery = reactive<SmisToolPersonalRequisitionSearchParams>({})
  const organizationOptions = ref<SmisToolScopeOption[]>([])
  const autoConfirmDays = ref(3)
  const overview = reactive<SmisToolPersonalRequisitionOverview>({
    total: 0,
    pending: 0,
  ...
  const validateSameEmployee = (rows: SmisToolPersonalRequisitionItem[]): boolean => {
  ...
  const openPushDialog = (rows: SmisToolPersonalRequisitionItem[]): void => {
  ...
  const confirmRows = async (
    rows: SmisToolPersonalRequisitionItem[],
    confirmed: boolean
  ): Promise<void> => {
    if (!rows.length || !validateSameEmployee(rows)) return
    ...
    if (!confirmed) {
      ...
      await confirmAction(`确认已领取选中的 ${rows.length} 项工器具？`, {
        title: '领用确认',
        confirmButtonText: '确认已领用'
      })
    }
    await confirmToolRequisitionItems(
      rows.map((row) => row.id),
      confirmed,
      reason
    )
  ...
  const headerActions = computed<ArtTableQueryHeaderAction[]>(() => [
    {
      permission: 'SmisToolPersonalRequisition:Generate',
      key: 'generate',
      label: '生成到期领用单',
      ...
      onClick: async () => {
        await generateDueToolRequisitions(dayjs().format('YYYY-MM-DD'))
        await refreshData()
      }
    },
    {
      ...
      onClick: ({ selectedRows }) =>
        openPushDialog(selectedRows as SmisToolPersonalRequisitionItem[])
    },
    {
      permission: 'SmisToolPersonalRequisition:Confirm',
      key: 'confirm',
      label: '确认领用',
      icon: 'ri:checkbox-circle-line',
      selectionRequired: true,
      onClick: ({ selectedRows }) =>
        confirmRows(selectedRows as SmisToolPersonalRequisitionItem[], true)
    },
    {
      permission: 'SmisToolPersonalRequisition:Confirm',
      key: 'deny',
      ...
      selectionRequired: true,
      onClick: ({ selectedRows }) =>
        confirmRows(selectedRows as SmisToolPersonalRequisitionItem[], false)
    },
    {
      permission: 'SmisToolPersonalRequisition:Export',
      type: 'export',
      label: '导出',
      exportFilename: '工器具个人领用',
      ...
      exportApi: async ({ maxRows }) => ({
        data: (
          await fetchToolPersonalRequisitionList({
            ...searchQuery,
            purpose: 'export',
            from: 0,
            to: maxRows - 1
    ...
    {
      permission: 'SmisToolPersonalRequisition:Statistics',
      key: 'statistics',
      label: '统计分析',
    ...
    {
      permission: 'SmisToolPersonalRequisition:Configure',
      key: 'configure',
      label: '自动确认设置',
  ...
  const columnsFactory = (): ColumnOption<SmisToolPersonalRequisitionItem>[] => [
            ...
            list={[
              {
                ...
                label: '确认领用',
                icon: 'ri:checkbox-circle-line',
                auth: 'SmisToolPersonalRequisition:Confirm',
                disabled:
  ...
  const fetchTableData = async (params: TableParams) => {
    const result = await fetchToolPersonalRequisitionList(normalizePaginationParams(params))
