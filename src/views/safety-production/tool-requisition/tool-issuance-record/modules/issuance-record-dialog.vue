<template>
  <ArtDialog ref="dialogRef" size="xl">
    ...
    <ArtSectionCard title="工器具明细" subtitle="发放数量必须大于 0；物料信息来自工器具主数据。">
      <template #actions>
      ...
      <ArtTable
        ...
        table-layout="fixed"
        empty-text="暂无发放明细"
        empty-description="点击新增明细选择需要发放的工器具。"
      />
    </ArtSectionCard>
  </ArtDialog>
</template>
...
<script setup lang="tsx">
  ...
  import {
    ...
    type SmisToolIssuanceRecordItem,
  ...
  interface FormModel {
    ...
    issueDate: string
    remark: string
    items: SmisToolIssuanceRecordItem[]
  }
  ...
  const detailColumns: ColumnOption<SmisToolIssuanceRecordItem>[] = [
    {
      prop: 'materialName',
      label: '物料信息',
      minWidth: 220,
  ...
  const materialSnapshot = (item: SmisToolIssuanceRecordItem): SmisMaterial => ({
    id: item.materialId,
    tenantId: getUserInfo.value.tenantId || '',
    materialCode: '',
