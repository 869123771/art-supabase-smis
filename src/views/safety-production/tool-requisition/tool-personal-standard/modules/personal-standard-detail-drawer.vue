    ...
    <div v-if="employee" class="tool-detail">
      <section class="tool-detail__identity" aria-label="员工个人标准概览">
        ...
        <div class="tool-detail__summary">
          ...
          <span
            ><small>工器具定额</small><strong>{{ items.length }} 项</strong></span
          >
          <span
      ...
      >
        <template #actions>
          <ElTag type="primary" effect="plain">{{ items.length }} 项工器具</ElTag>
        </template>

        <div class="tool-detail__list">
          <article v-for="item in items" :key="item.id" class="tool-detail__item">
            <div class="tool-detail__material">
              <ElImage
                v-if="item.imageUrls?.[0]"
                class="tool-detail__image"
                ...
                preview-teleported
              />
              <span v-else class="tool-detail__image is-empty" aria-hidden="true">
            ...
            <dl class="tool-detail__facts">
              ...
              <div>
                ...
                <dd>
                  <ArtDictDisplay
                    dict-code="smisToolIssuanceCycle"
...
<script setup lang="ts">
  ...
  const drawerRef = ref<ArtDrawerExpose<SmisToolPersonalStandard>>()
  const employee = ref<SmisToolPersonalStandard>()
  const items = ref<SmisToolPersonalStandardItem[]>([])
  ...
  const loadItems = async (): Promise<void> => {
    ...
    error.value = null
    try {
      const result = await fetchToolPersonalStandardItems(employee.value.employeeId)
      if (result.error) throw result.error
      items.value = result.data ?? []
    } catch (reason) {
      items.value = []
      error.value = getFriendlySupabaseErrorMessage(
        reason,
        '工器具个人标准明细加载失败，请稍后重试'
      )
    } finally {
