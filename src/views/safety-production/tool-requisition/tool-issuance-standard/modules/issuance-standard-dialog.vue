    ...
    <div class="issuance-dialog">
      ...
      >
        <template #positionIds>
          ...
          >
            <template #empty><SmisDataSourceEmptyActions source="position" /></template>
          </ArtTableMultipleSelect>
        </template>
      ...
      >
        <template #actions>
          <ArtTableMultipleSelect
            ...
            empty-text="暂无可选的工器具物料"
            empty-description="当前租户没有匹配的启用工器具。请在物料信息中新增物料，将物料类型设为“工器具”并启用；仅在现有类别不适用时再维护类别。"
...
<script setup lang="tsx">
  ...
  import {
    ...
    type SmisToolIssuanceStandardDetail,
    type SmisToolIssuanceStandardSavePayload,
    type SmisToolScopeOption,
    type SmisToolStandardStatus
  } from '@smis/api'

  export interface IssuanceStandardDialogOpenData {
    row?: SmisToolIssuanceStandard
  }
  interface FormModel extends Omit<SmisToolIssuanceStandardSavePayload, 'details'> {
    details: SmisToolIssuanceStandardDetail[]
  }
  ...
  const selectedOrganizations = ref<SmisToolScopeOption[]>([])
  ...
  const detailColumns = computed<ColumnOption<SmisToolIssuanceStandardDetail>[]>(() => [
    {
      prop: 'materialName',
      label: '物料信息',
      minWidth: 200,
  ...
  const normalizeScopeRows = (rows: DataSelectRecord[]): SmisToolScopeOption[] =>
    rows as SmisToolScopeOption[]
  const fetchPositions = async (params: DataSelectFetchParams) => {
    const result = await fetchToolScopeOptions('position', params.keyword)
    ...
    form.details = materials.map(
      (material, index) =>
        current.get(material.id) ?? {
          ...
          status: 'enabled' as SmisToolStandardStatus,
          remark: '',
          sort: (index + 1) * 10
        }
    )
  ...
  const handleOpen = async (data: IssuanceStandardDialogOpenData) => {
    ...
    await dialogRef.value?.handleOpen(data, {
      title: data.row ? '编辑工器具发放标准' : '新增工器具发放标准',
      subtitle: '配置适用范围、发放节奏与工器具明细',
      confirmText: '保存标准',
