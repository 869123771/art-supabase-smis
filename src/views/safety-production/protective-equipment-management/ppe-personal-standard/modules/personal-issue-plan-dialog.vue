<template>
  <ArtDialog ref="dialogRef" size="lg">
    ...
    <ArtSectionCard
      ...
      empty-title="暂无个人用品标准"
      empty-description="请先为该员工生成个人防护用品标准。"
    >
      <ArtTable
        :data="form.items"
        :columns="planColumns"
...
<script setup lang="tsx">
  ...
  interface PlanItem extends SmisPpePersonalStandardItem {
    initialIssueDate: string
    nextIssueDate: string
  }

  const emit = defineEmits<{ success: [] }>()
  const dialogRef = ref<ArtDialogExpose<SmisPpePersonalStandard>>()
  const employee = shallowRef<SmisPpePersonalStandard>()
  ...
  const handleSubmit = async (): Promise<boolean> => {
    ...
    if (!form.items.length) {
      ElMessage.warning('请先生成个人防护用品标准')
      return false
    }
    ...
    try {
      await savePpePersonalIssuePlan(
