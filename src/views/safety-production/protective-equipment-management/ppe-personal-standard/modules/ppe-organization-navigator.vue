<template>
  ...
  >
    <template #actions>
      <ArtTreeExpandToggle :tree="treeRef" :data="data" label="组织树" />
      <ArtIconButton
    ...
    <div
      class="ppe-org-nav__content smis-organization-navigator__content smis-category-navigator__content"
    >
      <ElInput
        v-model="keyword"
        clearable
      ...
      <button
        type="button"
        class="ppe-org-nav__all smis-organization-navigator__all smis-category-navigator__all"
      ...
      <div
        class="ppe-org-nav__section-label smis-organization-navigator__section-label smis-category-navigator__section-label"
      >
        <span>组织结构</span><small>{{ organizationCount }} 个有效节点</small>
      </div>

      <ElScrollbar
        class="ppe-org-nav__scrollbar smis-organization-navigator__scrollbar smis-category-navigator__scrollbar"
      >
        <ElTree
          ref="treeRef"
          :data="data"
        ...
        >
          <template #default="{ data: node }">
            <div
              class="ppe-org-nav__node smis-organization-navigator__node smis-category-navigator__node"
            >
              ...
              <ArtSvgIcon
                ...
                class="ppe-org-nav__check smis-organization-navigator__check smis-category-navigator__check"
                icon="ri:check-line"
                aria-hidden="true"
              />
            </div>
      ...
      <footer
        class="ppe-org-nav__selection smis-organization-navigator__selection"
        aria-live="polite"
      >
...
<script setup lang="ts">
  ...
  import type { SmisPpeScopeOption } from '@smis/api'

  const ALL_KEY = 'all'
  const props = defineProps<{
    data: SmisPpeScopeOption[]
    loading: boolean
    error: string | null
    selectedKey: string
  ...
  const filterNode = (value: string, data: TreeNodeData): boolean => {
    const node = data as SmisPpeScopeOption
  ...
  const getOrganizationIcon = (node: SmisPpeScopeOption): string =>
    ({
  ...
  const handleNodeClick = (node: SmisPpeScopeOption): void => emit('select', node.id)
