<template>
  ...
  >
    ...
    <div class="risk-organization__content">
      <ElInput
        v-model="keyword"
        clearable
      ...
      <div class="risk-organization__summary">
        <span aria-hidden="true"><ArtSvgIcon icon="ri:organization-chart" /></span>
        <span
          ><strong>组织结构</strong><small>{{ organizationCount }} 个有效节点</small></span
        >
      </div>

      <ElScrollbar class="risk-organization__scrollbar">
        <ElTree
          ref="treeRef"
          :data="data"
          :props="treeProps"
        ...
        >
          <template #default="{ data: node }">
            <div class="risk-organization__node">
              ...
              <ArtSvgIcon
                ...
                class="risk-organization__check"
                icon="ri:check-line"
                aria-hidden="true"
              />
            </div>
...
<script setup lang="ts">
  ...
  const syncCurrentNode = async (): Promise<void> => {
    await nextTick()
    treeRef.value?.setCurrentKey(props.selectedKey || undefined)
  }
